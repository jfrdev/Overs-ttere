...
struct
  ...
  fun checkStat s vtable ftable rtable b =
	...
	then
          ...
          in
            if
              comparertable rtable1' andalso comparertable rtable2'
            then 
              checkStat s1 vtable ftable rtable b
              @ checkStat s2 vtable ftable rtable b
            else 
              checkStat s1 vtable ftable rtable false
              @ checkStat s2 vtable ftable rtable false
          end
	else raise Error ("Condition should be integer",p) 
      ...
      (*farlig*)
      | S100.Block (ds,ss,p) => (checkStats ss ((checkDecs ds)@vtable) ftable rtable b)
